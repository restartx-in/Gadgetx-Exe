class BrandRepository {
    ...
    async getAllByTenantId(db, tenantId, filters = {}) {
        ...
        let query = `
            SELECT b.*, db.name as done_by_name, cc.name as cost_center_name
            FROM "brand" b
        ...
        if (tenantId) {
            whereClauses.push(`b.tenant_id = $${paramIndex++}`);
            params.push(tenantId);
        }

        const filterConfig = {
            name: { operator: 'ILIKE', column: 'b.name' },
            done_by_id: { operator: '=', column: 'b.done_by_id' },
            cost_center_id: { operator: '=', column: 'b.cost_center_id' },
        };
        ...
        if (sort) {
            ...
            if (dbColumn) {
                query += ` ORDER BY ${dbColumn} ${direction}, b.id DESC`;
            } else {
    ...
    async update(db, id, tenantId = null, data) {
        ...
        let query = `UPDATE "brand" SET ${setClause} WHERE id = $${fields.length + 1}`;
    ...
    async delete(db, id, tenantId = null) {
        let query = `DELETE FROM "brand" WHERE id = $1`;
        const params = [id];
