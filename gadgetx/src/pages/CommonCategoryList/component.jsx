...
import {
  Table, Thead, Tbody, Tr, Td, Th, ThSL, TdSL, TdMenu, ThMenu,
...
const CommonCategoryList = ({
  ...
  const handleAddClick = useCallback(() => {
    setSearchParams((prev) => { prev.set("action", "add"); return prev; }, { replace: true });
  }, [setSearchParams]);

  const handleDelete = useCallback(async (id) => {
    try {
      await deleteCategory(id);
      showToast({ crudItem: CRUDITEM.CATEGORY, crudType: CRUDTYPE.DELETE_SUCCESS });
    } catch (error) {
      showToast({ crudItem: CRUDITEM.CATEGORY, crudType: CRUDTYPE.DELETE_ERROR });
    }
  }, [deleteCategory, showToast]);

  const handleRefresh = useCallback(() => {
    const reset = {
      name: "", sort: "", done_by_id: "", 
      cost_center_id: defaultCostCenter, searchType: "", searchKey: ""
    };
    setState(reset);
        ...
        {!isMobile ? (
          <>
            ...
            <TableTopContainer
              mainActions={
                <>
                  <PopUpFilter isOpen={showFilter} setIsOpen={setShowFilter} onApply={() => setState(uiState)}>
                    <VStack>
                      <InputField label="Name" value={uiState.name} onChange={(e) => setUiState({...uiState, name: e.target.value})} />
                      <DoneByAutoComplete value={uiState.done_by_id} onChange={(e) => setUiState({...uiState, done_by_id: e.target.value})} />
                      <CostCenterAutoComplete value={uiState.cost_center_id} disabled={isDisableCostCenter} onChange={(e) => setUiState({...uiState, cost_center_id: e.target.value})} />
                    </VStack>
                  </PopUpFilter>
                  <RefreshButton onClick={handleRefresh} />
                  <PopupSearchField
                    searchRef={searchRef} searchKey={uiState.searchKey} setSearchKey={(v) => setUiState({...uiState, searchKey: v})}
                    searchType={uiState.searchType} setSearchType={(v) => setUiState({...uiState, searchType: v})}
                    handleSearch={() => setState(uiState)} searchOptions={searchOptions}
                  />
                        ...
                        <ThFilterContainer>
                          ...
                          <ThSearchOrFilterPopover isSearch popoverWidth={200}>
                            <InputField 
                              ...
                              onKeyDown={(e) => e.key === 'Enter' && setState({name: uiState.name})}
                              isLabel={false} 
                            />
            ...
            <ScrollContainer>
              <PageHeader>
                <HStack>
                   ...
                   <RefreshButton onClick={handleRefresh} />
                   <MobileSearchField 
                      searchKey={uiState.searchKey} setSearchKey={(v) => setUiState({...uiState, searchKey: v})}
                      handleSearch={() => setState(uiState)} searchOptions={searchOptions}
                   />
                </HStack>
                <AddButton onClick={handleAddClick}>Add Category</AddButton>
              </PageHeader>
              {isLoading ? <Loader /> : listData.length === 0 ? <TableCaption item={categoryItemConstant} /> : (
                <div>
                  {listData.map((item) => (
                    <ListItem
                      key={item.id}
                      title={item.name}
                      subtitle={<><div>By: {item.done_by_name}</div><div>CC: {item.cost_center_name}</div></>}
                      onView={() => setModal({ isOpen: true, mode: "view", item })}
                      onEdit={() => setModal({ isOpen: true, mode: "edit", item })}
                      onDelete={() => handleDelete(item.id)}
                    />
