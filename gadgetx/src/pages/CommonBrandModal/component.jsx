import { useEffect, useMemo, useRef } from "react";
import { useForm, Controller } from "react-hook-form";
import { zodResolver } from "@hookform/resolvers/zod";
import { z } from "zod";

import InputField from "@/components/InputField";
import { Modal, ModalHeader, ModalFooter, ModalBody } from "@/components/Modal";
import VStack from "@/components/VStack";
import { useToast } from "@/context/ToastContext";
import { CRUDTYPE, CRUDITEM } from "@/constants/object/crud";
import { TOASTTYPE, TOASTSTATUS } from "@/constants/object/toastType";
import { Transaction } from "@/constants/object/transaction";
import Title from "@/components/Title";
import { Report } from "@/constants/object/report";
import { onFormError } from "@/utils/formUtils";
import CancelButton from "@/components/CancelButton";
import SubmitButton from "@/components/SubmitButton";
import DeleteTextButton from "@/components/DeleteTextButton";

// Auto-completes
import DoneByAutoCompleteWithAddOption from "@/apps/user/components/DoneByAutoCompleteWithAddOption";
import CostCenterAutoCompleteWithAddOption from "@/apps/user/components/CostCenterAutoCompleteWithAddOption";

const DRAFT_STORAGE_KEY = "brand_form_draft";

// Define Zod Schema
const brandSchema = z.object({
  name: z.string().min(1, "Brand name is required"),
  done_by_id: z.union([z.string(), z.number()]).optional().nullable(),
  cost_center_id: z.union([z.string(), z.number()]).optional().nullable(),
});

const CommonBrandModal = ({
  useCreateHook,
  useUpdateHook,
  useDeleteHook,
  isOpen,
  onClose,
  mode,
  selectedItem,
  onItemCreated,
}) => {
  const showToast = useToast();
  const disabled = mode === "view";
  
  // Ref to track if we successfully submitted to block race-condition saves
  const isSuccessRef = useRef(false);

  const { mutateAsync: createItem, isPending: creating } = useCreateHook();
  const { mutateAsync: updateItem, isPending: updating } = useUpdateHook();
  const { mutateAsync: deleteItem, isPending: deleting } = useDeleteHook();

  const defaultValues = useMemo(() => ({
    name: "",
    done_by_id: null,
    cost_center_id: null,
  }), []);

  const { 
    control, 
    handleSubmit, 
    reset, 
    watch, 
    setFocus,
    formState: { isSubmitting, isDirty } // Using isDirty to prevent saving reset forms
  } = useForm({
    resolver: zodResolver(brandSchema),
    defaultValues,
  });

  const watchedFields = watch();

  // 1. Sync Form State Logic
  useEffect(() => {
    if (isOpen) {
      isSuccessRef.current = false; // Reset success flag when opening

      if (mode === "edit" || mode === "view") {
        reset({
          ...defaultValues,
          ...selectedItem,
        });
      } else {
        // ADD MODE
        const savedDraft = localStorage.getItem(DRAFT_STORAGE_KEY);
        // Priority: Autocomplete Typed Value > Draft > Default
        if (selectedItem?.name) {
          reset({ ...defaultValues, name: selectedItem.name });
        } else if (savedDraft) {
           try {
             reset(JSON.parse(savedDraft));
           } catch (e) {
             reset(defaultValues);
           }
        } else {
          reset(defaultValues);
        }
      }

      if (mode !== "view") {
        setTimeout(() => setFocus("name"), 100);
      }
    } else {
      // Clean up when closed
      reset(defaultValues);
    }
  }, [isOpen, mode, selectedItem, reset, setFocus, defaultValues]);

  // 2. Draft Persistence Logic
  useEffect(() => {
    // Only save if:
    // 1. We are in Add mode and Open
    // 2. Not currently submitting
    // 3. Form is DIRTY (User actually typed something)
    // 4. We haven't just successfully submitted (isSuccessRef)
    if (
      mode === "add" && 
      isOpen && 
      !isSubmitting && 
      isDirty && 
      !isSuccessRef.current
    ) {
      localStorage.setItem(DRAFT_STORAGE_KEY, JSON.stringify(watchedFields));
    }
  }, [watchedFields, mode, isOpen, isSubmitting, isDirty]);

  const handleFormError = (errors) => onFormError(errors, showToast);

  const onFormSubmit = async (data) => {
    const payload = {
      name: data.name,
      done_by_id: data.done_by_id ? Number(data.done_by_id) : null,
      cost_center_id: data.cost_center_id ? Number(data.cost_center_id) : null,
    };

    try {
      if (mode === "edit") {
        await updateItem({ id: selectedItem.id, data: payload });
        showToast({
          crudItem: CRUDITEM.BRAND,
          crudType: CRUDTYPE.UPDATE_SUCCESS,
        });
      } else {
        const newItem = await createItem(payload);
        showToast({
          crudItem: CRUDITEM.BRAND,
          crudType: CRUDTYPE.CREATE_SUCCESS,
        });
        
        // Mark as success to block the "Save Draft" effect
        isSuccessRef.current = true;

        // Immediate cleanup
        localStorage.removeItem(DRAFT_STORAGE_KEY);
        reset(defaultValues);

        if (onItemCreated) onItemCreated(newItem.data || newItem);
      }
      onClose();
    } catch (err) {
      showToast({
        type: TOASTTYPE.GENARAL,
        message: err.response?.data?.error || "An unexpected error occurred.",
        status: TOASTSTATUS.ERROR,
      });
    }
  };

  const handleDelete = async () => {
    if (!selectedItem?.id) return;
    try {
      await deleteItem(selectedItem.id);
      showToast({
        crudItem: CRUDITEM.BRAND,
        crudType: CRUDTYPE.DELETE_SUCCESS,
      });
      localStorage.removeItem(DRAFT_STORAGE_KEY);
      onClose();
    } catch (error) {
      showToast({
        type: TOASTTYPE.GENARAL,
        message: error.response?.data?.error || "Failed to delete brand.",
        status: TOASTSTATUS.ERROR,
      });
    }
  };

  return (
    <Modal isOpen={isOpen} onClose={onClose} size="sm">
      <ModalHeader>
        <Title report={Report.Brand} mode={mode} />
      </ModalHeader>
      <form onSubmit={handleSubmit(onFormSubmit, handleFormError)}>
        <ModalBody>
          <VStack>
            <Controller
              name="name"
              control={control}
              render={({ field }) => (
                <InputField
                  {...field}
                  label="Brand Name"
                  disabled={disabled}
                  required
                />
              )}
            />

            <Controller
              name="done_by_id"
              control={control}
              render={({ field }) => (
                <DoneByAutoCompleteWithAddOption
                  {...field}
                  placeholder="Select Done By"
                  disabled={disabled}
                />
              )}
            />

            <Controller
              name="cost_center_id"
              control={control}
              render={({ field }) => (
                <CostCenterAutoCompleteWithAddOption
                  {...field}
                  placeholder="Select Cost Center"
                  disabled={disabled}
                />
              )}
            />
          </VStack>
        </ModalBody>
      </form>
      <ModalFooter
        style={{
          width: "100%",
          display: "flex",
          justifyContent: "flex-end",
          gap: "16px",
        }}
      >
        {mode === "add" && <CancelButton onClick={onClose} />}
        {mode === "edit" && (
          <DeleteTextButton
            transaction={Transaction.Brand}
            onDelete={handleDelete}
            isLoading={deleting}
          />
        )}
        <SubmitButton
          isLoading={creating || updating}
          disabled={disabled}
          type={mode}
          onClick={handleSubmit(onFormSubmit, handleFormError)}
        />
      </ModalFooter>
    </Modal>
  );
};

export default CommonBrandModal;