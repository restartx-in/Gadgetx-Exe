...
const mopSchema = z.object({
  ...
  default_ledger_id: z.union([z.string(), z.number()]).optional().nullable(),
});
  ...
  useEffect(() => {
    if (isOpen) {
      if (mode === "edit" || mode === "view") {
        ...
        // If autocomplete provided a typed name, prioritize it over draft
      ...
      <form onSubmit={handleSubmit(onFormSubmit, handleFormError)}>
        <ModalBody>
          <VStack>
            ...
            {LedgerSelectorComponent && (
              <Controller
                name="default_ledger_id"
                control={control}
                render={({ field }) => (
                  <LedgerSelectorComponent
                    {...field}
                    placeholder="Default Ledger"
                    disabled={disabled}
