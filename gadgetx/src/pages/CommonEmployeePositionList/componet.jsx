...
const EmployeePositionRow = React.memo(
  ({ type, index, onEdit, onView, onDelete }) => {
    return (
      <Tr>
        <TdSL index={index} page={1} pageSize={0} />
        <Td>{type.name}</Td>
...
const CommonEmployeePositionList = ({
  list,
  isLoading,
  deleteItem,
  refetch,
  ...
  const ENTITY_CRUD_KEY = "EMPLOYEE_POSITION";
  const PAGE_TITLE = "Employee Position";
  const ADD_BUTTON_TEXT = "Add Employee Position";
  const ENTITY_TRANSACTION_CONSTANT = CRUDITEM.EmployeePosition;
  ...
  const handleSearch = useCallback(() => {
    ...
    handleSetURLState({ ...urlState, ...filterReset, searchType, searchKey });
  }, [
    searchType,
    searchKey,
    urlState,
  ...
  const handleFilter = useCallback(() => {
    ...
    setShowFilter(false);
  }, [name, doneById, costCenterId, handleSetURLState, urlState]);
  ...
  return (
    <>
      ...
      <AddModalComponent
        ...
        mode={mode}
        selectedEmployeePosition={selectedItem}
        onEmployeePositionCreated={handleItemCreated}
      />
    </>
  );
};
