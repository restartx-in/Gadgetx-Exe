...
async function fetchMonthlyReport(filters) {
  ...
  const res = await api.get(
    `${API_ENDPOINTS.LEDGERS.BASE}/report/monthly${query}`
  );
  return res.data;
}

async function fetchDailyReport(filters) {
  const query = buildQueryParams(filters);
  const res = await api.get(
    `${API_ENDPOINTS.LEDGERS.BASE}/report/daily${query}`
  );
  return res.data;
}
...
export function useLedgerReport(filters = {}) {
  ...
  const isDaily = filters.view === "daily";
