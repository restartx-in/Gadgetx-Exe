...
export const useExportCommon = () => {
  ...
  const maps = useMemo(
    () => ({
      customers: customersData.reduce(
        (acc, item) => ({ ...acc, [item.id]: item.name }),
        {}
      ),
      accounts: accountsData.reduce(
        (acc, item) => ({ ...acc, [item.id]: item.name }),
        {}
      ),
      doneBys: doneBysData.reduce(
        (acc, item) => ({ ...acc, [item.id]: item.name }),
        {}
      ),
      costCenters: costCentersData.reduce(
        (acc, item) => ({ ...acc, [item.id]: item.name }),
        {}
      ),
    }),
  ...
  const getAcutalValueFromObject = (key, data) => {
    switch (key) {
      ...
      case "depositDateEnd":
        return data && typeof data.format === "function"
  ...
  const getSearchTypeData = (searchType, searchKey) => {
    if (searchKey !== "")
      return `${getSearchTypeName(searchType)}:${searchKey}`;
    else return "";
  };
