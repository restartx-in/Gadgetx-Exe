...
const RegisterSession = () => {
  ...
  const paymentModeRows = stats?.mode_of_payment_breakdown || [];

  const endDate = closed_at
    ? closed_at
    : (() => {
        const now = new Date();
        const pad = (n) => n.toString().padStart(2, "0");
  ...
  const statsRows = [
    ...
    {
      label: "Total Sale Returns",
      amount: stats.sale_return_total,
      type: "minus",
    },
    {
    ...
    {
      label: "Sale Profit",
      amount: stats.sale_profit,
      type: stats.sale_profit >= 0 ? "plus" : "minus",
    },
    { label: "Total Purchases", amount: stats.purchase_total, type: "minus" },
    {
      label: "Total Purchase Returns",
      amount: stats.purchase_return_total,
      type: "plus",
    },
    ...
    {
      label: "Cash Balance",
      amount: stats.closing_cash_balance,
      type: "neutral",
    },
    {
      label: "Account Balance",
      amount: stats.closing_bank_balance,
      type: "neutral",
    },
  ];
  ...
  return (
    <ContainerWrapper className="sale-page">
      ...
      <ScrollContainer>
        <div className="sale-page__form-content">
          ...
          <div className="sale-page__top-controls fs16 fw600">
            ...
            <InputField
              label="Done By"
              value={session.done_by_name || "N/A"}
              readOnly
            />
            <InputField
              label="Cost Center"
              value={session.cost_center_name || "N/A"}
              readOnly
            />
          </div>
          ...
          <div className="sale-page__payments-panel-top fs16 fw600">
            <HStack spacing={4}>
              <InputField
                label="Opening Note"
                value={session.opening_note || "-"}
                readOnly
              />
              <InputField
                label="Closing Note"
                value={session.closing_note || "-"}
                readOnly
              />
            </HStack>
          </div>
          ...
          <div className="sale-page__order-table">
            <div className="section-title mb-2 fs16 fw600">
              Session Breakdown
            </div>
            <Table>
              <Thead>
              ...
              <Tbody>
                {statsRows.map((row, index) => (
                  <Tr key={index}>
                    <Td>{row.label}</Td>
                    <Td>
                      <AmountSymbol>{row.amount}</AmountSymbol>
                    </Td>
                  </Tr>
                ))}
              </Tbody>
          ...
          <div className="sale-page__order-table">
            ...
            <Table>
              ...
              <Tbody>
                {paymentModeRows.length > 0 ? (
                  paymentModeRows.map((row) => (
                    <Tr key={row.mode_of_payment_id || row.mode_of_payment_name}>
                      <Td>{row.mode_of_payment_name}</Td>
                      <Td>{row.transaction_count}</Td>
                      <Td>
                        <AmountSymbol>{row.total_amount}</AmountSymbol>
                      </Td>
                    </Tr>
                  ))
                ) : (
                  <Tr>
                    <Td colSpan={3}>No payment mode transactions found for this session.</Td>
                  </Tr>
                )}
              </Tbody>
            </Table>
      ...
      <div className="sale-page__bottom-section">
        <div
          className="sale-page__summary-panel fs16 fw600"
          ...
          {isClosed && (
            <>
              <HStack justifyContent="space-between" className="total-row">
                ...
                <span className="total-value">
                  <AmountSymbol>{session.closing_cash}</AmountSymbol>
                </span>
              </HStack>
