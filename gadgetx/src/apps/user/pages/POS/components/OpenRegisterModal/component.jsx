...
import CostCenterAutoCompleteWithAddOption from '@/apps/user/components/CostCenterAutoCompleteWithAddOption' // Added Import
import SubmitButton from '@/apps/user/components/SubmitButton'
import { useToast } from '@/context/ToastContext'
import { TOASTTYPE, TOASTSTATUS } from '@/constants/object/toastType'
import useCreateRegisterSession from '@/hooks/api/registerSession/useCreateRegisterSession'
...
const OpenRegisterModal = ({ isOpen, onClose, onRegisterOpened }) => {
  ...
  const defaultForm = {
    done_by_id: '',
    cost_center_id: defaultCostCenter, // Set default cost center
    opening_cash: '',
    opening_note: ''
  }
  const [form, setForm] = useState({ ...defaultForm })

  const { mutateAsync: openSession, isPending: isOpening } = useCreateRegisterSession()
  ...
  const handleSubmit = async () => {
    ...
    try {
      const payload = {
        ...
        cost_center_id: form.cost_center_id || null, // Pass cost center
      ...
      if (msg.includes("already has an open session")) {
          showToast({ type: TOASTTYPE.GENARAL, message: 'Session restored.', status: TOASTSTATUS.INFO })
          onClose() 
      } else {
  ...
  return (
    ...
    >
      ...
      <ModalBody className="register-modal__body">
        <p className="modal-instruction fs16">Please open a register session to continue.</p>
        
        <div className="form-group">
           <DoneByAutoCompleteWithAddOption
                value={form.done_by_id}
                onChange={(e) => setForm(prev => ({ ...prev, done_by_id: e.target.value }))}
        ...
        <InputField
             ...
             onChange={(e) => setForm(prev => ({ ...prev, opening_note: e.target.value }))}
        />
      </ModalBody>
