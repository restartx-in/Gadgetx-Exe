...
const UserSettingsModal = ({ isOpen, onClose, userId, onSuccess }) => {
  ...
  const { data: settings, isLoading: isLoadingSettings } = useFetchSettings(userId);
  const { mutateAsync: updateSettings, isLoading: isSaving } = useUpdateSettings();
  
  useEffect(() => {
    if (settings) {
      setForm({
        default_cost_center_id: settings.user_settings?.default_cost_center_id || '',
      });
    }
  }, [settings]);
