...
const LedgerRow = React.memo(({ row, index, page, pageSize, isDetailedMode, handlers, onRowClick }) => {
  const menuItems = useMemo(() => handlers.getMenuItems(row), [row, handlers]);
  ...
  return (
    <Tr onClick={() => onRowClick(row)} style={{ cursor: "pointer", transition: "background-color 0.2s" }}>
      ...
      {isDetailedMode ? (
        <>
           ...
           <TdNumeric style={{ color: row.credit > 0 ? "#1f2937" : "#d1d5db" }}>
              {parseFloat(row.credit || 0) !== 0 ? parseFloat(row.credit).toFixed(2) : "-"}
           </TdNumeric>
           <TdNumeric style={{ fontWeight: 500 }}>
              {row.balance !== undefined ? parseFloat(row.balance).toFixed(2) : "-"}
           </TdNumeric>
        </>
      ) : (
        <>
            <Td>{row.from_ledger_name}</Td>
            <Td>{row.to_ledger_name}</Td>
...
const MobileLedgerCard = React.memo(({ row, isDetailedMode, handlers, onRowClick }) => {
  const menuItems = useMemo(() => handlers.getMenuItems(row), [row, handlers]);
  ...
  const subtitle = (
    <div style={{ display: "flex", flexDirection: "column", fontSize: "12px", color: "#6b7280" }}>
        <span>{row.voucher_no} • <span style={{fontWeight:600}}>{displayType}</span></span>
        {row.invoice_no && <span>Inv: {row.invoice_no}</span>}
        {isDetailedMode && <span style={{fontWeight: 600, color: "#555"}}>Part: {row.particular_name}</span>}
        {row.description && <span>{row.description}</span>}
        {!isDetailedMode && <span>{row.from_ledger_name} ➔ {row.to_ledger_name}</span>}
    </div>
  );
  ...
  const amountDisplay = (
    <div style={{ textAlign: "right" }}>
        {isDetailedMode ? (
        <>
            {parseFloat(row.debit) > 0 && <div style={{ color: "#16a34a", fontSize: "12px" }}>Dr: {parseFloat(row.debit).toFixed(2)}</div>}
            {parseFloat(row.credit) > 0 && <div style={{ color: "#dc2626", fontSize: "12px" }}>Cr: {parseFloat(row.credit).toFixed(2)}</div>}
            <div style={{ fontWeight: "bold", color: "#1f2937", marginTop: "4px" }}>Bal: {parseFloat(row.balance).toFixed(2)}</div>
        </>
        ) : (
            <div style={{ fontWeight: "bold", color: "#1f2937" }}>{parseFloat(row.amount).toFixed(2)}</div>
        )}
    </div>
  );
...
const LedgerReport = () => {
  ...
  const [localLedgerId, setLocalLedgerId] = useState(searchParams.get("ledgerId") || "");
  const [localVoucherType, setLocalVoucherType] = useState(searchParams.get("voucherType") || "");
  ...
  const ledgerOptions = useMemo(() => 
    Array.isArray(ledgersList) ? ledgersList.map((l) => ({ value: l.id, label: l.name })) : [],
    [ledgersList]
  );

  const { data: reportData, isLoading, isRefetching, refetch } = useLedgerReport(state);
  ...
  const handleRowClick = useCallback((row) => {
    ...
    if (transactionId) {
      if (type === "SALERETURN") return navigate(`/sale-return/view/${transactionId}`);
      if (type === "PURCHASERETURN") return navigate(`/purchase-return/view/${transactionId}`);
      if (type === "SALE") return navigate(`/sale/view/${transactionId}`);
      if (type === "PURCHASE") return navigate(`/purchase/view/${transactionId}`);
    }
    ...
    getMenuItems: (v) => [
      {
        label: "Edit Voucher",
        onClick: () => {
           const type = (v.invoice_type || "").toUpperCase().replace(/\s+/g, "");
           const transactionId = v.invoice_no;

           if (transactionId) {
             if (type === "SALERETURN") return navigate(`/sale-return/edit/${transactionId}`);
             if (type === "PURCHASERETURN") return navigate(`/purchase-return/edit/${transactionId}`);
             if (type === "SALE") return navigate(`/sale/edit/${transactionId}`);
             if (type === "PURCHASE") return navigate(`/purchase/edit/${transactionId}`);
           }
           navigate(`/voucher/${v.id}`); 
        },
      },
      { label: "Delete", onClick: () => setVoucherToDelete(v), isDelete: true },
    ],
  }), [navigate]);

  const selectedLedgerName = useMemo(
    () => state.ledger_id 
        ? ledgerOptions.find((l) => String(l.value) === String(state.ledger_id))?.label || "Unknown Ledger"
        : "All Vouchers",
  ...
  const confirmDelete = useCallback(async () => {
    ...
    try {
      ...
      showToast({ crudItem: CRUDITEM.VOUCHER, crudType: CRUDTYPE.DELETE_SUCCESS });
      setVoucherToDelete(null);
      refetch();
    } catch (e) {
      showToast({ crudItem: CRUDITEM.VOUCHER, crudType: CRUDTYPE.DELETE_ERROR });
    }
  ...
  return (
    <ContainerWrapper>
      {!isMobile ? (
        <>
          <PageTitleWithBackButton title="Ledger Report" subtitle={selectedLedgerName} />
          <TableTopContainer
             isMargin={true}
             summary={
                isDetailedMode && !loading && (
                  <div style={{ display: "flex", gap: "1.5rem", alignItems: "center" }}>
                    <div style={{ fontSize: "14px", color: "#666" }}>
                      Opening: <span style={{ fontWeight: "bold", color: "#333" }}>{parseFloat(reportData?.opening_balance || 0).toFixed(2)}</span>
                    </div>
                  </div>
                )
             }
            mainActions={
              <>
                <DateFilter
                  ...
                  onChange={(v) => setState({ start_date: v.startDate, end_date: v.endDate, page: 1 })}
                />
                 <div style={{ minWidth: "220px" }}>
                  <SelectField
                    placeholder="All Vouchers"
                    options={[{ value: "", label: "All Vouchers" }, ...ledgerOptions]}
                    value={state.ledger_id}
                    onChange={(e) => setState({ ledger_id: e.target.value, page: 1 })}
                    isSearchable={true}
                  />
                </div>
                {/* Advanced Filter Popover */}
                <PopUpFilter isOpen={showFilter} setIsOpen={setShowFilter} onApply={handleFilterApply}>
                  <VStack spacing={4}>
                    <SelectField
                        label="Ledger"
                        placeholder="All Vouchers"
            ...
            <Table>
              <Thead>
                <Tr>
                  ...
                  <Th>
                    <ThContainer>
                      Date
                      <ThSort sort={state.sort} value="date" handleSort={handleSort} />
                    </ThContainer>
                  </Th>
                  ...
                  <Th>
                    <ThContainer>
                      Voucher No
                      <ThFilterContainer>
                        <ThSort sort={state.sort} value="voucher_no" handleSort={handleSort} />
                        <ThSearchOrFilterPopover isSearch={true} onSearch={() => handleHeaderSearch("voucher_no", headerFilters.voucher_no)}>
                            <InputField
                                value={headerFilters.voucher_no}
                                onChange={(e) => setHeaderFilters(prev => ({ ...prev, voucher_no: e.target.value }))}
                                onKeyDown={(e) => handleHeaderKeyDown(e, "voucher_no")}
                                isLabel={false}
                  ...
                  <Th>
                     <ThContainer>
                        Inv Type
                        <ThFilterContainer>
                           <ThSort sort={state.sort} value="voucher_type" handleSort={handleSort} />
                           ...
                           <ThSearchOrFilterPopover>
                              <div style={{ minWidth: '150px' }}>
                                <SelectField
                                    ...
                                    onChange={(e) => setState({ voucher_type: e.target.value, page: 1 })}
                                />
                              </div>
                           </ThSearchOrFilterPopover>
                        </ThFilterContainer>
                  ...
                  {isDetailedMode && (
                     <Th>
                        <ThContainer>
                           Particulars
                           <ThFilterContainer>
                              <ThSort sort={state.sort} value="particular_name" handleSort={handleSort} /> 
                              <ThSearchOrFilterPopover isSearch={true} onSearch={() => handleHeaderSearch("particular_name", headerFilters.particular_name)}>
                                 <InputField
                                    value={headerFilters.particular_name}
                                    onChange={(e) => setHeaderFilters(prev => ({ ...prev, particular_name: e.target.value }))}
                                    onKeyDown={(e) => handleHeaderKeyDown(e, "particular_name")}
                                    isLabel={false}
                    ...
                    <>
                        <Th>
                           <ThContainer>
                              Payment From
                              <ThFilterContainer>
                                 <ThSort sort={state.sort} value="from_ledger_name" handleSort={handleSort} />
                                 <ThSearchOrFilterPopover isSearch={true} onSearch={() => handleHeaderSearch("from_ledger_name", headerFilters.from_ledger_name)}>
                                    <InputField
                                       value={headerFilters.from_ledger_name}
                                       onChange={(e) => setHeaderFilters(prev => ({ ...prev, from_ledger_name: e.target.value }))}
                                       onKeyDown={(e) => handleHeaderKeyDown(e, "from_ledger_name")}
                                       isLabel={false}
                        ...
                        <Th>
                           <ThContainer>
                              Payment To
                              <ThFilterContainer>
                                 <ThSort sort={state.sort} value="to_ledger_name" handleSort={handleSort} />
                                 <ThSearchOrFilterPopover isSearch={true} onSearch={() => handleHeaderSearch("to_ledger_name", headerFilters.to_ledger_name)}>
                                    <InputField
                                       value={headerFilters.to_ledger_name}
                                       onChange={(e) => setHeaderFilters(prev => ({ ...prev, to_ledger_name: e.target.value }))}
                                       onKeyDown={(e) => handleHeaderKeyDown(e, "to_ledger_name")}
                                       isLabel={false}
                        ...
                        <Th>
                            <ThContainer>
                                Amount
                                <ThFilterContainer>
                                    <ThSort sort={state.sort} value="amount" handleSort={handleSort} />
                                </ThFilterContainer>
                            </ThContainer>
                        </Th>
                    </>
              ...
              <Tbody>
                {rows.length > 0 ? (
                  ...
                  <TableCaption item="Transactions" noOfCol={isDetailedMode ? 10 : 9} />
                )}
              </Tbody>
            </Table>
          )}
