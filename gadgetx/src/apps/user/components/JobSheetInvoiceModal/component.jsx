  ...
  useEffect(() => {
    if (isOpen && invoiceData) {
      ...
      if (printSettingsString) {
        ...
        // 2. Resolve Image URLs (safe build to avoid malformed URLs)
        const fullHeader = buildUploadUrl(
          API_UPLOADS_BASE,
          settings.header_image_proxy_path || settings.header_image_url,
        );
        if (fullHeader) settings.full_header_image_url = fullHeader;
        const fullQr = buildUploadUrl(API_UPLOADS_BASE, settings.qr_image_proxy_path || settings.qr_image_url);
        if (fullQr) settings.full_qr_image_url = fullQr;
    ...
    <div className="JobSheet_custom-modal__overlay" style={{ zIndex: "10000" }}>
      <div className="JobSheet_custom-modal__container">
        ...
        >
          ...
          <Button variant="print" onClick={handlePrintClick}>
            Print
          </Button>
        </div>
      </div>
