...
const JobSheetPDFModal = ({ isOpen, onClose, jobSheetId }) => {
    ...
    const handleDownloadPDF = async () => {
        ...
        try {
            const blob = await pdf(<JobSheetPDF jobSheetData={jobSheetData} />).toBlob();
            ...
            return (
                <>
                    <div className="receipt-modal__meta-details">
                        <p><strong>Job ID:</strong> {jobSheetData.job_id}</p>
                        <p><strong>Date:</strong> {new Date(jobSheetData.created_at).toLocaleDateString("en-GB")}</p>
                        <p><strong>Customer:</strong> {jobSheetData.party_name}</p>
                        
                        <p><strong>Item:</strong> {jobSheetData.item_name}</p>
                        <p><strong>Status:</strong> {jobSheetData.status}</p>
                    </div>
                    ...
                    <div className="receipt-modal__summary">
                        <div className="receipt-modal__summary-row grand-total">
                            ...
                            <span className="value">{formatCurrency(jobSheetData.service_charges)}</span>
                        </div>
                    </div>
    ...
    const isDownloadDisabled = isLoading || !!error || !jobSheetData || isGenerating;

    return (
        // Using the same class names as ReceiptModal for consistent styling
        <div className="receipt-modal__overlay" onClick={onClose} role="dialog" aria-modal="true">
            <div className="receipt-modal__content" onClick={(e) => e.stopPropagation()}>
                <div className="receipt-modal__header">
                    ...
                    <button className="receipt-modal__close-btn-icon" onClick={onClose} aria-label="Close">
                        &times;
                    </button>
                </div>
