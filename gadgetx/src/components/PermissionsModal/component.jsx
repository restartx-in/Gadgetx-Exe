...
const PermissionsModal = ({ isOpen, onClose, onSave, initialPermissions }) => {
  ...
  const handleSelectAll = (action) => {
    setPermissions((prev) => {
      const newPermissions = JSON.parse(JSON.stringify(prev));
      const areAllChecked = ALL_MODULES.every((module) => newPermissions[module]?.[action]);
  ...
  const getHeaderCheckboxState = (action) => {
    ...
    const checkedCount = ALL_MODULES.filter(module => permissions[module]?.[action]).length;
    if (checkedCount === 0) return { checked: false, indeterminate: false };
    if (checkedCount === totalModules) return { checked: true, indeterminate: false };
            ...
            <Thead>
              <Tr>
                ...
                {PERMISSION_ACTIONS.map((action) => {
                  const { checked, indeterminate } = getHeaderCheckboxState(action);
                  return (
              ...
              {PERMISSION_GROUPS.map((group) => (
                <React.Fragment key={group.title}>
                  <Tr className="group-header-row">
                    <Td colSpan={PERMISSION_ACTIONS.length + 1}>{group.title}</Td>
                  </Tr>
