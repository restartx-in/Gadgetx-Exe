  ...
  const handleConfirmPrint = () => {
    ...
    if (variant === 'jobsheet') {
      ...
      if (showName || showPrice) {
        const namePart = showName && itemName ? `<span class="item-name">${itemName}</span>` : '<span></span>';
        const pricePart = showPrice && formattedPrice ? `<span class="item-price">Price: ${formattedPrice}</span>` : '';
        itemDetailsHtml = `<div class="item-details">${namePart}${pricePart}</div>`;
      }

      innerContentHtml = `
        ${showStore && storeName ? `<div class="store-name">${storeName}</div>` : ''}
        ${itemDetailsHtml}
        ${barcodeSvgString}
      `;
    }
