  ...
  const getTableData = (isPdf = false) => {
    if (listData && listData.length) {
      const data = listData.map((item, index) => {
        ...
        const accountName = item.account_name || maps?.accounts?.[item.account_id] || "";

        return {
          SL: SL,
          Date: item.created_at,
          Account: accountName,
          Type: item.transaction_type ? item.transaction_type.replace(/_/g, " ") : "",
          "Done By": doneByName,
          "Cost Center": costCenterName,
          Description: cleanDescription(item.description),
          Debit: parseFloat(item.debit || 0) > 0 ? parseFloat(item.debit).toFixed(2) : "-",
          Credit: parseFloat(item.credit || 0) > 0 ? parseFloat(item.credit).toFixed(2) : "-",
        };
      });

      // Add Balance Row if an account is filtered and total is provided
      if (totalData && totalData.balance !== undefined && totalData.balance !== null) {
        const totalRow = {
          SL: "",
          Date: "Total Balance",
          Account: "",
